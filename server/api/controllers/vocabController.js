...
const search_vocab = async (req, res) => {
   try {
       const { english, german, spanish } = req.query;
       const query = {};
       if (english) query.english = { $regex: english, $options: 'i' }; // Case-insensitive regex search
       if (german) query.german = { $regex: german, $options: 'i' };
       if (spanish) query.spanish = { $regex: spanish, $options: 'i' };
 
       // Find the first matching document
       const result = await vocabModel.findOne(query);
